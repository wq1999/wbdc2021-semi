...
def PLE(dnn_feature_columns, num_tasks, task_types, task_names, num_levels=2, num_experts_specific=8,
        num_experts_shared=4, dnn_units=[256, 128, 64], expert_dnn_units=[64, 64], gate_dnn_units=None, tower_dnn_units_lists=[[16, 16], [16, 16]],
    ...
    # share-bottom
    share_bottom_outs = DNN(dnn_units, dnn_activation, l2_reg_dnn, dnn_dropout, dnn_use_bn, seed=seed,
                                     name='task_' + 'share_bottom')(dnn_input)
    ...
    def cgc_net(inputs, level_name, is_last=False):
        ...
        for i in range(num_tasks):
            ...
            expert_concat = tf.keras.layers.Concatenate(axis=1,
                        name=level_name + 'expert_concat_specific_' + task_names[i])(cur_experts)
            ...
            gate_out = Lambda(lambda x: tf.tile(x, [1, 1, expert_dnn_units[-1]]))(gate_out)
            ...
            gate_mul_expert = Lambda(lambda x: tf.math.reduce_sum(x, axis=1))(gate_mul_expert)
        ...
        if not is_last:
            ...
            expert_concat = tf.keras.layers.Concatenate(axis=1,
                            name=level_name + 'expert_concat_shared_' + task_names[i])(cur_experts)
            # expert_concat = tf.keras.layers.concatenate(cur_experts, axis=1,
    ...
    for task_type, task_name, tower_dnn, ple_out in zip(task_types, task_names, tower_dnn_units_lists, ple_outputs):
        ...
        # share bottom
        tower_output1 = DNN(tower_dnn, dnn_activation, l2_reg_dnn, dnn_dropout, dnn_use_bn, seed=seed,
                           name='share_bottom_' + task_name)(share_bottom_outs)
        logit1 = tf.keras.layers.Dense(1, use_bias=False, activation=None)(tower_output1)
        output1 = PredictionLayer(task_type, name=task_name + '_share_bottom')(logit1)

        outputs = tf.keras.layers.Add(name=task_name + '_final')([output, output1])

        task_outs.append(outputs)
